...
static llvm::Error Evaluate_DW_OP_deref(
  ...
  // Handle deref of a register location.
  // When the current value is a register location description then a deref
  // operation should read the value from the register. We have eagerly
  // read the register value already and its value is on top of the stack
  // so we just need to reset the value context and description to their
  // defaults.
  if (dwarf4_location_description_kind == Register) {
    ...
    // The register value is already on top of the stack so there is nothing
    // more to do here.
    return llvm::Error::success();
  }
...
llvm::Expected<Value> DWARFExpression::Evaluate(
  ...
  while (opcodes.ValidOffset(offset)) {
    ...
    case DW_OP_deref_size: {
      ...
      // Deref a register location and truncate the value to `size` bytes.
      // See the corresponding comment in DW_OP_deref for more details
      // on why we deref a register this way.
      if (dwarf4_location_description_kind == Register) {
        // Reset context to default values.
        dwarf4_location_description_kind = Memory;
        stack.back().ClearContext();

        // Truncate the register value on top of the stack to size bytes then
