...
public:
  ...
  // Accessors
  size_t GetRegisterCount() const { return m_register_count; }
  size_t GetRegisterBufferSize() const { return m_register_buffer_size; }
  uint32_t GetPCRegisterNumber() const { return m_pc_register_num; }

  const RegisterInfo *GetRegisterInfoAtIndex(size_t reg) const;
  const RegisterSet *GetRegisterSet(size_t set_index) const;
  size_t GetRegisterSetCount() const { return g_register_sets.size(); }
...
private:
  ...
  size_t m_register_buffer_size = 0;
  uint32_t m_pc_register_num = 0;

  // Static shared register information (shared across all instances for the
  // same architecture)
  static std::vector<RegisterInfo> g_reg_infos;
  static std::vector<RegisterSet> g_register_sets;
